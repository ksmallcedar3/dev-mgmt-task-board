    ...
    <>
        ...
        <SidebarContent className="px-1 py-3 group-data-[collapsible=icon]:hidden">
          {departments.map((dept) => (
            <SidebarGroup key={dept.id} className="px-1">
              ...
              >
                <Plus />
                <span className="sr-only">{dept.name} にカテゴリーを追加</span>
              </SidebarGroupAction>
              <SidebarGroupContent>
                <SidebarMenu>
                  {dept.positions.map((pos) => {
                    const active = pos.id === selectedPositionId;
                    return (
      ...
      {addDialogDept && (
        <AddItemDialog
          ...
          title="カテゴリーを追加"
          description={`${addDialogDept.name} に新しいカテゴリーを追加します`}
          fieldLabel="カテゴリー名"
          fieldId="pos-name"
          placeholder="例: インフラ"
